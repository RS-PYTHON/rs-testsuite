...
def step_check_s3_configuration(context):
    ...
    assert (
        "S3_ACCESS_KEY" in os.environ
    ), "S3_ACCESS_KEY environment variable is not set."
    assert (
        "S3_SECRET_ACCESS_KEY" in os.environ
    ), "S3_SECRET_ACCESS_KEY environment variable is not set."
    assert (
        "S3_DOMAIN_URL" in os.environ
    ), "S3_DOMAIN_URL environment variable is not set."
...
def step_s3_bucket_exist(context, bucket: str):
    assert (
        s3_bucket_existence(context, bucket) is True
    ), f"The bucket {bucket} does not exist."
...
def step_s3_file_bucket_exist(context, file_path: str, bucket: str):
    assert (
        s3_file_bucket_existence(context, file_path, bucket) is True
...
def step_s3_file_bucket_dont_exist(context, file_path: str, bucket: str):
    assert (
        s3_file_bucket_existence(context, file_path, bucket) is False
    ), f"The file {file_path} exist on the bucket {bucket}."


def assert_s3_client_is_set(context):
    assert (
        context.s3_client is not None
    ), "s3 client is not configured (call 'given s3 access is configured' first)."


def s3_bucket_existence(context, bucket: str) -> bool:
    """Return true if the S3 bucket exists."""
    assert_s3_client_is_set(context)
    result = False
    try:
...
def step_s3_file_creation(context, file_path: str, bucket: str):
    ...
    try:
        context.s3_client.upload_file(temp, bucket, file_path)
        os.remove(temp)
