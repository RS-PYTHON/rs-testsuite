...
def step_given_stac_collections_exist(context, collections: str):
    context.stac_collections = collections.split(',')
    ...
    matches = 0
    for id in context.stac_collections:
        for fc in found_collections:
            if fc.id == os.environ['USER'] + '_' + id:
                matches += 1
                break

    assert matches == len(context.stac_collections), f"No expected count of collections found for {collections}"
...
def step_given_stac_items_exist(context, items: str):
    ...
    found_items = create_pystac_client(context).get_items(*context.stac_items)
    assert found_items is not None, f"No items found for {items}"
    assert len(list(found_items)) == len(context.stac_items), f"No expected count of items found for {items}"
